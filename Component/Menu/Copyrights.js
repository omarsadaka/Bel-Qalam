      ...
      getData(){
        ...
        NetInfo.fetch().then(state =>{
          if (state.isConnected){
            try {
              axios.get('http://165.22.83.141/api/user/copyWrite',{
              }).then(response => {
       ...
       renderOption(lang){
         if (lang === 'AR'){
            return (
                <View style={[styles.shadow,{width:width,height:'8%',alignItems:'center',backgroundColor:'#118CB3',flexDirection:'row'}]}>
                 ...
                 <Icon name="chevron-left" size={18} color="#fff" style={{marginStart:10}} />
                  ...
                  <Text style={{width:'80%',color:'#fff',textAlign:'center',fontSize:this.normalize(22),fontFamily:'segoe'}}>حقـوق النشر</Text>
                 <TouchableOpacity
            ...
            return (
              ...
              <Text style={{width:'80%',color:'#fff',textAlign:'center',fontSize:this.normalize(22),fontFamily:'segoe'}}>Copyrights</Text>
             <TouchableOpacity
             style={{width:'10%',height:20}} />
           </View>
            );
